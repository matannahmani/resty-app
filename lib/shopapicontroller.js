...
const apipatchShop = async (props,shopid) => 
      await axios.patch(`${process.env.API_URL}/shops/${shopid}`, {shop: {name: props.name},user: {name: props.users[0].name,phone: props.users[0].phone,email: props.users[0].email}})
        .then(res => ({
          code: 200,
          data: res.data,
      }))
...
export {shopstatus,switchdelivery,switchshop,switchtakeaway,newshop,apigetShops,apideleteShop,apipatchShop};